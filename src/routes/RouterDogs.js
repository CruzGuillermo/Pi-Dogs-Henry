...
router.post("/", async (req, res) => {
  ...
  if (!name || !heightMin || !heightMax || !weightMin || !weightMax)
    return res.status(404).json("Faltan informacon obligatoria");
  try {
...
router.put("/:id/edit", async (req, res) => {
  const { id } = req.params
  try {
    const editDog = await Dog.findByPk(id);

    if (Object.keys(editDog).length) {
      ...
      if (req.body.temperament) {
        //seteo los temperamentos solamente si me los pasan x body (si no lo hago tira error)
        const temperamentAux = await Temperament.findAll({
          where: {
            name: req.body.temperament,
          },
        });
        editDog.setTemperaments(temperamentAux);
      }

      res.status(200).send("Perro editado con exito!");
    } else res.status(404).send("Perro con id no encontrado");
  } catch (e) {
    console.log("error PUT try catch", e);
    res.status(400).send("Id mal escrito");
  }
});
